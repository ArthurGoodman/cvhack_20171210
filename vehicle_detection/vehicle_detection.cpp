...
int main(int argc, char **argv) {
    std::cout << "Using OpenCV " << CV_MAJOR_VERSION << "." << CV_MINOR_VERSION << "."
              << CV_SUBMINOR_VERSION << std::endl;

    CvCapture *capture;
    IplImage *frame;
    int input_resize_percent = 75;
    ...
    frame = cvCreateImage(
        cvSize(
            ...
            (int)((frame1->height * input_resize_percent) / 100)),
        frame1->depth,
        frame1->nChannels);
...
void detect(IplImage *img) {
    ...
    CvSeq *object = cvHaarDetectObjects(
        ...
        cvSize(120, 120) // cvSize(70,70)//cvSize(640,480)  //---------MAXSIZE
        );
